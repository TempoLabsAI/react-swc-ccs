...
function App() {
  ...
  const generateCheckoutSession = useAction(api.subscriptions.createCheckoutSession);
  ...
  const handleStripe = useCallback(async () => {
    const session = await generateCheckoutSession({
      priceId: "price_1QY9x0KCuFqcLnh8MySw9jMP"
    });

    if (session?.url) {
      window.location.assign(session.url); // Redirect to Stripe Checkout session?.url!;
    };
